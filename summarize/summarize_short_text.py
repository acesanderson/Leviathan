...
- The first summary should be long (~"""
    + str(chain_of_density_summary_length_in_words)
    + """  words) yet highly non-specific, 
containing little information beyond the entities marked as missing. Use overly verbose language and fillers
(e.g., "this article discusses") to reach ~"""
    + str(chain_of_density_summary_length_in_words)
    + """ words.
...
Remember, use the exact same number of words for each summary (around """
    + str(chain_of_density_summary_length_in_words)
    + """ words).
...
def chain_of_convergence(text: str, number_of_summaries: int = 3) -> str:
    ...
    response = chain.run(
        input_variables={
            "number": number_of_summaries,
            "summaries": enumerated_summaries,
        },
        verbose=False,
    )
